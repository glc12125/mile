...
class ObsManager(ObsManagerBase):
    ...
    def attach_ego_vehicle(self, parent_actor):
        ...
        with h5py.File(maps_h5_path, 'r', libver='latest', swmr=True) as hf:
            ...
            self._lane_marking_all = np.array(
                hf['lane_marking_all'], dtype=np.uint8)
            ...
            assert np.isclose(self._pixels_per_meter,
    ...
    def _get_stops(criteria_stop):
        ...
        if (stop_sign is not None) and (not criteria_stop._stop_completed):
            ...
            stops = [
                (carla.Transform(trans.location, trans.rotation), bb_loc, bb_ext)]
        return stops
    ...
    def get_observation(self):
        ...
        lane_mask_all = cv.warpAffine(
            self._lane_marking_all, M_warp, (self._width, self._width)).astype(np.bool)
        ...
        cv.polylines(route_mask, [np.round(route_warped).astype(
    ...
    def _get_mask_from_actor_list(self, actor_list, M_warp):
        ...
        for actor_transform, bb_loc, bb_ext in actor_list:
            ...
            cv.fillConvexPoly(mask, np.round(
                corners_warped).astype(np.int32), 1)
        return mask.astype(np.bool)
    ...
    def _get_warp_transform(self, ev_loc, ev_rot):
        ...
        bottom_left = ev_loc_in_px - self._pixels_ev_to_bottom * \
            forward_vec - (0.5*self._width) * right_vec
        top_left = ev_loc_in_px + \
            (self._width-self._pixels_ev_to_bottom) * \
            forward_vec - (0.5*self._width) * right_vec
